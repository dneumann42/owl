...
pub const While = struct { condition: *Ast, program: *Ast };
pub const For = struct { variable: *Ast, iterable: *Ast, program: *Ast };
...
pub fn whilex(allocator: std.mem.Allocator, cond: *Ast, blk: *Ast, meta: Meta) !*Ast {
    return new(allocator, .{ .whilex = .{ .program = blk, .condition = cond } }, meta);
}

pub fn forx(allocator: std.mem.Allocator, variable: *Ast, iterable: *Ast, blk: *Ast, meta: Meta) !*Ast {
    return new(allocator, .{ .forx = .{ .variable = variable, .iterable = iterable, .program = blk } }, meta);
}
...
pub fn program(allocator: std.mem.Allocator, meta: Meta) !*Ast {
    return new(allocator, .{ .program = std.ArrayList(*Ast).init(allocator) }, meta);
}
